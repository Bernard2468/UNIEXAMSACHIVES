...
/**
 * Shared base for the family of leave application forms — Catholic University
 * of Ghana, Fiapre-Sunyani. All leave forms (annual, casual, …) have the same
 * 15-item paper structure and the same workflow:
 *
 *   1. officer       — Section 1-13 (Officer fills the application and signs)
 *   2. recommender   — Section 14 (Dean/HOD/Director OR Head of any Office)
 *   3. registrar     — Section 15 (Registrar approves and signs)
 *
 * Subclasses only need to declare their identity (slug, code, title,
 * description, view paths). Stages, footer instruction and any field
 * tweaks live here so the family stays in lock-step.
 *
 * To add a new leave type (e.g. Study Leave):
 *   1. Create a subclass of this base with its own slug/code/title.
 *   2. Register it in AppServiceProvider.
 *   3. Add a thin PDF wrapper that includes
 *      `admin.forms.pdf._leave-application`.
 * The renderer, controllers, sidebar, gallery and PDF pipeline pick it up.
 */
abstract class BaseLeaveApplicationForm extends BaseFormDefinition
{
    /**
     * The standing HR note that prints at the bottom of every leave form.
     * Identical across the leave family because it instructs the Head of
     * HR on what to do with the form regardless of which leave type it is.
     */
