                                ...
                                <div class="col-xl-12">
                                    <div class="compose-panel">
                                        ...
                                        <div id="form-request-hint" style="display:none;margin:10px 0;padding:10px 14px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;color:#1e3a8a;font-size:13px;line-height:1.5;">
                                            ...
                                            This is a <strong>form request</strong> — address it to <strong>exactly one</strong> recipient (the approver). Add anyone else who should see it as <strong>Cc</strong>, and optionally route it <strong>Through</strong> someone if necessary.
                                        </div>
...
.lhpk-thumb-check {
  ...
  background:#1a4a9b; color:#fff;
  display:none; align-items:center; justify-content:center;
  font-size:10px;
  box-shadow:0 2px 6px rgba(26,74,155,.4);
}
.lhpk-thumb.is-active .lhpk-thumb-check { display:flex; }

/* Scroll arrows (only show when rail overflows) */
.lhpk-nav {
...
.lhpk-preview-tag {
  font-size:11px; font-weight:700; color:#1a4a9b;
...
@media (max-width: 600px) {
  .lhpk-thumb { flex-basis:84px; }
  .lhpk-nav { display:none !important; }
}
...
/* Show scroll arrows only when the rail overflows */
(function() {
    const rail = document.getElementById('lhpk-rail');
    const wrap = rail ? rail.parentElement : null;
    if (!rail || !wrap) return;
    function checkOverflow() {
        wrap.classList.toggle('has-overflow', rail.scrollWidth > rail.clientWidth + 2);
    }
...
// Initialise selection (handles old() restore and default state)
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ===== THROUGH ===== */
    // Through is only valid in "Selected Users" mode — keep it in sync with the recipient type.
...
function selectAllCc() {
    document.querySelectorAll('.cc-checkbox').forEach(cb => cb.checked = true);
    updateCcCount();
}
...
function syncThroughVisibility() {
    ...
    if (!show) clearThrough(); // never submit a Through person when the picker is hidden
}
...
function toggleThroughSection() {
    ...
    btn.innerHTML = open
        ? '<i class="icofont-plus-circle"></i> Set a Through Person'
        : '<i class="icofont-minus-circle"></i> Hide Through';
}
