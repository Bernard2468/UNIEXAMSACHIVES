...
- Only an institutional admin may change them — via `HomeController::updateUserOrganization` (`users.update-organization`) and at creation in `storeUser`. The whole `#users` route block is gated by the `institutional_admin` middleware (server-side, not just sidebar hiding).
